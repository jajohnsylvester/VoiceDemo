...
import { FilterState, AiRecommendation, Voice } from './types';
...
const App: React.FC = () => {
  ...
  const [selectedAnimateVoice, setSelectedAnimateVoice] = useState<Voice | null>(null);
  ...
  const isModalOpen = showVoiceFinder || (aiResult && isAiCardVisible) || !!selectedAnimateVoice;
  ...
  return (
    <div className="h-screen w-screen bg-[#FDFDFD] dark:bg-[#09090b] text-zinc-900 dark:text-zinc-100 font-sans overflow-hidden flex flex-col relative transition-colors duration-300">
                  ...
                  viewMode === 'carousel' ? (
                    <div className="w-full flex-1 flex items-center justify-center pb-8 min-h-0">
                         <Carousel3D 
                            ...
                            onAnimate={setSelectedAnimateVoice}
      ...
      <AnimatePresence>
        {selectedAnimateVoice && (
          <AnimateModal 
              voice={selectedAnimateVoice}
              onClose={() => setSelectedAnimateVoice(null)}
          />
        )}
      </AnimatePresence>
